...
public class OpenMenu implements CommandExecutor {
    ...
    public static void handleConsoleHistory(CommandSender commandSender, OfflinePlayer target, Command command, String s, String[] strings) {
        ...
        */
        int lines = 0;
        for (String line : Main.plugin.getPlayerCommandLogs().getConfigurationSection("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData").getKeys(false)) {
            lines++;
        }
        ...
        if (strings[1].contains("last") && !strings[1].contains("monday") && !strings[1].contains("thursday") && !strings[1].contains("wednesday") && !strings[1].contains("tuesday") && !strings[1].contains("friday") && !strings[1].contains("saturday") && !strings[1].contains("sunday")) {
            ...
            for (String line : Main.plugin.getPlayerCommandLogs().getConfigurationSection("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData").getKeys(false)) {
                if (lines >= (amount - (orderAmount))) {
                    ...
                    commandSender.sendMessage("       Command: " + Main.plugin.getPlayerCommandLogs().getString("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData." + line + ".Command"));
                    commandSender.sendMessage("       Worldname: " + Main.plugin.getPlayerCommandLogs().getString("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData." + line + ".WorldName"));
                    commandSender.sendMessage("       Position: " + Main.plugin.getPlayerCommandLogs().getString("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData." + line + ".Position"));
                }
                lines++;
            }
        ...
        */
        ...
        for (int i = 0; i <= 44; i++) {
            if (datesPos < dates.size() && dates.size() != 0) {
                ...
                lore.add(ChatColor.DARK_PURPLE + "Command: " + ChatColor.YELLOW + Main.plugin.getPlayerCommandLogs().getString("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData." + dates.get(datesPos) + ".Command"));
                lore.add(ChatColor.DARK_PURPLE + "Worldname: " + ChatColor.YELLOW + Main.plugin.getPlayerCommandLogs().getString("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData." + dates.get(datesPos) + ".WorldName"));
                lore.add(ChatColor.DARK_PURPLE + "Position: " + ChatColor.YELLOW + Main.plugin.getPlayerCommandLogs().getString("PlayerCommandLogs." + target.getUniqueId().toString() + ".CommandData." + dates.get(datesPos) + ".Position"));
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (command.getName().equalsIgnoreCase("commandhistory")) {
            if (commandSender.hasPermission("bouwserver.commands.commandhistory")) {
                ...
                if (strings.length == 0) {
                    commandSender.sendMessage(Messages.STAFF_TAG + ChatColor.RED + "Je moet wel een speler opgeven!");
                    return false;
                }
                ...
                if (strings.length > 0) {
                    ...
                    try {
                        if (Bukkit.getPlayer(PlayerCache.getUUIDFromPlayerName(strings[0].toLowerCase())) != null) {
                            target = Bukkit.getPlayer(PlayerCache.getUUIDFromPlayerName(strings[0].toLowerCase()));
                        } else if (Bukkit.getOfflinePlayer(PlayerCache.getUUIDFromPlayerName(strings[0].toLowerCase())) != null) {
                            target = Bukkit.getOfflinePlayer(PlayerCache.getUUIDFromPlayerName(strings[0].toLowerCase()));
                        }
                    } catch (NullPointerException exc) {
                        commandSender.sendMessage(Messages.STAFF_TAG + ChatColor.RED + "Kan de opgegeven speler niet vinden. Hij staat mogelijk niet in het systeem geregistreerd");
                        return false;
